...
export const AdminSideNavPanel: React.FC<IProps> = ({ open }) => {
  const { navItems, handleItemClick, handleSubItemClick } =
    useSideNavState(adminSideNavItems);
...
const NavLinkItem: React.FC<{
  itemKey: string;
  item: ISideNavItem;
  handleItemClick: ItemclickType;
}> = ({ item, itemKey, handleItemClick }) => {
...
type SubItemclickType = (
  event: any,
  itemKey: string,
  subItemIndex: number
) => void;

const NavLinkItemSubs: React.FC<{
  itemKey: string;
  item: ISideNavItem;
  handleSubItemClick: SubItemclickType;
}> = ({ itemKey, item, handleSubItemClick }) => {
  return (
    <>
...
const NavSubItem: React.FC<{
  ...
  return (
    <ListItem
      onClick={(e) => handleSubItemClick(e, itemKey, subIndex)}
      sx={{ pt: 0, pb: 0, pl: 4 }}
    >
      <ListItemButton>
